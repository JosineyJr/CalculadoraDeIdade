...
export class HomePage {
    ...
    calcularDiasProximoAniversario(idadeEmDias: number, dataNascimento: Date): void {
    ...
    calcularIdadeEmDias(dataAtual: Date, dataAniversario: Date): number {
        const idadeEmDias = parseInt(String((Number(dataAtual) - Number(new Date(dataAniversario))) / (24 * 3600 * 1000)));
    ...
    calcularProximaDataAniversario(): void {
        ...
        if (dataAniversario.getMonth() === currentDate.getMonth() &&
            ...
            this.weekDayNextBirthDay = this.weekDay(new Date(currentDate.getFullYear(),
                dataAniversario.getMonth(), dataAniversario.getDate()).getDay());
            console.log(new Date(currentDate.getFullYear(),
                dataAniversario.getMonth(), dataAniversario.getDate()));

        } else if (dataAniversario.getMonth() <= currentDate.getMonth()) {
            this.weekDayNextBirthDay = this.weekDay(new Date(currentDate.getFullYear() + 1, dataAniversario.getMonth(),
                dataAniversario.getDate()).getDay());
            console.log('aqui' + new Date(currentDate.getFullYear() + 1, dataAniversario.getMonth(),
                dataAniversario.getDate()));
        } else {
            this.weekDayNextBirthDay = this.weekDay(new Date(currentDate.getFullYear(), dataAniversario.getMonth(),
                dataAniversario.getDate()).getDay());
            console.log(new Date(currentDate.getFullYear(), dataAniversario.getMonth(),
                dataAniversario.getDate()));
        }
    }
